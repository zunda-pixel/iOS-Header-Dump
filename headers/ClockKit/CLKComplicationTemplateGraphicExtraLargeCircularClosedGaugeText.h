//
//   Generated by https://github.com/blacktop/ipsw (Version: 3.1.685, BuildCommit: 61fde72ba2e055974329fddcad4020b94632d855)
//
//    - LC_BUILD_VERSION:  Platform: iOS, MinOS: 26.5, SDK: 26.5, Tool: ld (1267)
//    - LC_SOURCE_VERSION: 2483.340.80.1.0
//
#ifndef CLKComplicationTemplateGraphicExtraLargeCircularClosedGaugeText_h
#define CLKComplicationTemplateGraphicExtraLargeCircularClosedGaugeText_h
@import Foundation;

#include "CLKComplicationTemplateGraphicExtraLargeCircular.h"
#include "CLKGaugeProvider.h"
#include "CLKTextProvider.h"

@interface CLKComplicationTemplateGraphicExtraLargeCircularClosedGaugeText : CLKComplicationTemplateGraphicExtraLargeCircular

@property (copy, nonatomic) CLKGaugeProvider *gaugeProvider;
@property (copy, nonatomic) CLKTextProvider *centerTextProvider;

/* class methods */
+ (id)templateWithGaugeProvider:(id)provider centerTextProvider:(id)provider;

/* instance methods */
- (void)_enumerateGaugeProviderKeysWithBlock:(id /* block */)block;
- (void)_enumerateTextProviderKeysWithBlock:(id /* block */)block;
- (id)initWithGaugeProvider:(id)provider centerTextProvider:(id)provider;

@end

#endif /* CLKComplicationTemplateGraphicExtraLargeCircularClosedGaugeText_h */
