//
//   Generated by https://github.com/blacktop/ipsw (Version: 3.1.685, BuildCommit: 61fde72ba2e055974329fddcad4020b94632d855)
//
//    - LC_BUILD_VERSION:  Platform: iOS, MinOS: 26.5, SDK: 26.5, Tool: ld (1267)
//    - LC_SOURCE_VERSION: 3525.4.1.0.0
//
#ifndef SISchemaDictationAlternativesPresent_h
#define SISchemaDictationAlternativesPresent_h
@import Foundation;

#include "SISchemaInstrumentationMessage.h"
#include "SISchemaLocaleIdentifier.h"

@interface SISchemaDictationAlternativesPresent : SISchemaInstrumentationMessage {
    /* instance variables */
    struct { unsigned int numberOfUnderlines:1; unsigned int countOfWordsUnderlined:1; unsigned int multilingualIsLowConfidence:1; } _has;
}

@property (nonatomic) int numberOfUnderlines;
@property (nonatomic) _Bool hasNumberOfUnderlines;
@property (nonatomic) int countOfWordsUnderlined;
@property (nonatomic) _Bool hasCountOfWordsUnderlined;
@property (retain, nonatomic) SISchemaLocaleIdentifier *alternativesLocale;
@property (nonatomic) _Bool hasAlternativesLocale;
@property (nonatomic) _Bool multilingualIsLowConfidence;
@property (nonatomic) _Bool hasMultilingualIsLowConfidence;
@property (readonly, nonatomic) NSData *jsonData;

/* instance methods */
- (_Bool)readFrom:(id)from;
- (unsigned long long)hash;
- (id)applySensitiveConditionsPolicy:(id)policy;
- (void)writeTo:(id)to;
- (id)initWithJSON:(id)json;
- (id)suppressMessageUnderConditions;
- (id)initWithDictionary:(id)dictionary;
- (id)dictionaryRepresentation;
- (_Bool)isEqual:(id)equal;
- (void)deleteAlternativesLocale;
- (void)deleteCountOfWordsUnderlined;
- (void)deleteMultilingualIsLowConfidence;
- (void)deleteNumberOfUnderlines;

@end

#endif /* SISchemaDictationAlternativesPresent_h */
