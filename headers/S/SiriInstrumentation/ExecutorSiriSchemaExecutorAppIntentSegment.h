//
//   Generated by https://github.com/blacktop/ipsw (Version: 3.1.685, BuildCommit: 61fde72ba2e055974329fddcad4020b94632d855)
//
//    - LC_BUILD_VERSION:  Platform: iOS, MinOS: 26.5, SDK: 26.5, Tool: ld (1267)
//    - LC_SOURCE_VERSION: 3525.4.1.0.0
//
#ifndef ExecutorSiriSchemaExecutorAppIntentSegment_h
#define ExecutorSiriSchemaExecutorAppIntentSegment_h
@import Foundation;

#include "SISchemaInstrumentationMessage.h"
#include "SISchemaUUID.h"

@interface ExecutorSiriSchemaExecutorAppIntentSegment : SISchemaInstrumentationMessage {
    /* instance variables */
    struct { unsigned int executorAppIntentTask:1; unsigned int executorAppIntentAttribution:1; unsigned int machTimeStart:1; unsigned int machTimeEnd:1; } _has;
}

@property (nonatomic) int executorAppIntentTask;
@property (nonatomic) _Bool hasExecutorAppIntentTask;
@property (nonatomic) int executorAppIntentAttribution;
@property (nonatomic) _Bool hasExecutorAppIntentAttribution;
@property (nonatomic) long long machTimeStart;
@property (nonatomic) _Bool hasMachTimeStart;
@property (nonatomic) long long machTimeEnd;
@property (nonatomic) _Bool hasMachTimeEnd;
@property (retain, nonatomic) SISchemaUUID *sessionUUID;
@property (nonatomic) _Bool hasSessionUUID;
@property (retain, nonatomic) SISchemaUUID *requestUUID;
@property (nonatomic) _Bool hasRequestUUID;
@property (readonly, nonatomic) NSData *jsonData;

/* instance methods */
- (_Bool)readFrom:(id)from;
- (unsigned long long)hash;
- (id)applySensitiveConditionsPolicy:(id)policy;
- (void)writeTo:(id)to;
- (id)initWithJSON:(id)json;
- (id)suppressMessageUnderConditions;
- (id)initWithDictionary:(id)dictionary;
- (id)dictionaryRepresentation;
- (_Bool)isEqual:(id)equal;
- (void)deleteExecutorAppIntentAttribution;
- (void)deleteExecutorAppIntentTask;
- (void)deleteMachTimeEnd;
- (void)deleteMachTimeStart;
- (void)deleteRequestUUID;
- (void)deleteSessionUUID;

@end

#endif /* ExecutorSiriSchemaExecutorAppIntentSegment_h */
