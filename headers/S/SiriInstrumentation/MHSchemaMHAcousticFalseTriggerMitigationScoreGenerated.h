//
//   Generated by https://github.com/blacktop/ipsw (Version: 3.1.685, BuildCommit: 61fde72ba2e055974329fddcad4020b94632d855)
//
//    - LC_BUILD_VERSION:  Platform: iOS, MinOS: 26.5, SDK: 26.5, Tool: ld (1267)
//    - LC_SOURCE_VERSION: 3525.4.1.0.0
//
#ifndef MHSchemaMHAcousticFalseTriggerMitigationScoreGenerated_h
#define MHSchemaMHAcousticFalseTriggerMitigationScoreGenerated_h
@import Foundation;

#include "SISchemaInstrumentationMessage.h"

@interface MHSchemaMHAcousticFalseTriggerMitigationScoreGenerated : SISchemaInstrumentationMessage {
    /* instance variables */
    struct { unsigned int score:1; unsigned int audioDurationInNs:1; unsigned int threshold:1; } _has;
}

@property (nonatomic) float score;
@property (nonatomic) _Bool hasScore;
@property (nonatomic) unsigned long long audioDurationInNs;
@property (nonatomic) _Bool hasAudioDurationInNs;
@property (nonatomic) float threshold;
@property (nonatomic) _Bool hasThreshold;
@property (readonly, nonatomic) NSData *jsonData;

/* instance methods */
- (_Bool)readFrom:(id)from;
- (unsigned long long)hash;
- (void)writeTo:(id)to;
- (id)initWithJSON:(id)json;
- (id)suppressMessageUnderConditions;
- (id)initWithDictionary:(id)dictionary;
- (id)dictionaryRepresentation;
- (_Bool)isEqual:(id)equal;
- (void)deleteThreshold;
- (void)deleteAudioDurationInNs;
- (void)deleteScore;

@end

#endif /* MHSchemaMHAcousticFalseTriggerMitigationScoreGenerated_h */
