//
//   Generated by https://github.com/blacktop/ipsw (Version: 3.1.685, BuildCommit: 61fde72ba2e055974329fddcad4020b94632d855)
//
//    - LC_BUILD_VERSION:  Platform: iOS, MinOS: 26.5, SDK: 26.5, Tool: ld (1267)
//    - LC_SOURCE_VERSION: 116.600.22.0.0
//
#ifndef SharedChannelProvisionStorageValue_h
#define SharedChannelProvisionStorageValue_h
@import Foundation;

@class PBCodable;

@interface SharedChannelProvisionStorageValue : PBCodable <NSCopying> {
    /* instance variables */
    struct { unsigned int pushPriority:1; } _has;
}

@property (retain, nonatomic) NSMutableArray *channelPublishProvisionPacketInfos;
@property (nonatomic) _Bool hasPushPriority;
@property (nonatomic) int pushPriority;

/* class methods */
+ (Class)channelPublishProvisionPacketInfoType;

/* instance methods */
- (_Bool)readFrom:(id)from;
- (void)copyTo:(id)to;
- (unsigned long long)hash;
- (void)mergeFrom:(id)from;
- (id)copyWithZone:(struct _NSZone *)zone;
- (void)writeTo:(id)to;
- (id)dictionaryRepresentation;
- (id)description;
- (_Bool)isEqual:(id)equal;
- (int)StringAsPushPriority:(id)priority;
- (void)addChannelPublishProvisionPacketInfo:(id)info;
- (id)channelPublishProvisionPacketInfoAtIndex:(unsigned long long)index;
- (unsigned long long)channelPublishProvisionPacketInfosCount;
- (void)clearChannelPublishProvisionPacketInfos;
- (id)pushPriorityAsString:(int)string;

@end

#endif /* SharedChannelProvisionStorageValue_h */
