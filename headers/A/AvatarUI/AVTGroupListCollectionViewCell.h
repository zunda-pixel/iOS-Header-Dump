//
//   Generated by https://github.com/blacktop/ipsw (Version: 3.1.685, BuildCommit: 61fde72ba2e055974329fddcad4020b94632d855)
//
//    - LC_BUILD_VERSION:  Platform: iOS, MinOS: 26.5, SDK: 26.5, Tool: ld (1267)
//    - LC_SOURCE_VERSION: 395.500.1.0.0
//
#ifndef AVTGroupListCollectionViewCell_h
#define AVTGroupListCollectionViewCell_h
@import Foundation;

@class UICollectionViewCell, UIImageView, UILabel, UIVisualEffectView;

@interface AVTGroupListCollectionViewCell : UICollectionViewCell

@property (readonly, nonatomic) UIVisualEffectView *visualEffectView;
@property (readonly, nonatomic) UILabel *label;
@property (readonly, nonatomic) UIImageView *imageView;
@property (copy, nonatomic) NSString *title;
@property (copy, nonatomic) NSString *symbolName;

/* class methods */
+ (id)highlightedBackgroundColor;
+ (id)cellIdentifier;
+ (id)defaultSymbolImageName;
+ (double)estimatedLabelWidthForContainerWidth:(double)width;

/* instance methods */
- (struct CGSize { double x0; double x1; })systemLayoutSizeFittingSize:(struct CGSize { double x0; double x1; })size withHorizontalFittingPriority:(float)priority verticalFittingPriority:(float)priority;
- (void)setSelected:(_Bool)selected;
- (id)initWithFrame:(struct CGRect { struct CGPoint { double x0; double x1; } x0; struct CGSize { double x0; double x1; } x1; })frame;
- (double)estimatedLabelWidth;
- (void)setupContent;

@end

#endif /* AVTGroupListCollectionViewCell_h */
