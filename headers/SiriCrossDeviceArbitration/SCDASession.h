//
//   Generated by https://github.com/blacktop/ipsw (Version: 3.1.685, BuildCommit: 61fde72ba2e055974329fddcad4020b94632d855)
//
//    - LC_BUILD_VERSION:  Platform: iOS, MinOS: 26.5, SDK: 26.5, Tool: ld (1267)
//    - LC_SOURCE_VERSION: 3520.58.1.0.0
//
#ifndef SCDASession_h
#define SCDASession_h
@import Foundation;

@interface SCDASession : NSObject <NSCopying, NSSecureCoding>

@property (readonly, nonatomic) unsigned long long generation;
@property (readonly, copy, nonatomic) NSUUID *sessionId;
@property (readonly, copy, nonatomic) NSUUID *currentElectionAdvertisementId;
@property (readonly, copy, nonatomic) NSData *currentElectionAdvertisementData;
@property (readonly, copy, nonatomic) NSDictionary *electionAdvertisementDataByIds;

/* class methods */
+ (id)newWithBuilder:(id /* block */)builder;
+ (_Bool)supportsSecureCoding;

/* instance methods */
- (id)_descriptionWithIndent:(unsigned long long)indent;
- (unsigned long long)hash;
- (id)copyWithZone:(struct _NSZone *)zone;
- (id)mutatedCopyWithMutator:(id /* block */)mutator;
- (id)initWithCoder:(id)coder;
- (id)initWithBuilder:(id /* block */)builder;
- (id)init;
- (void)encodeWithCoder:(id)coder;
- (id)description;
- (_Bool)isEqual:(id)equal;
- (id)initWithGeneration:(unsigned long long)generation sessionId:(id)id currentElectionAdvertisementId:(id)id currentElectionAdvertisementData:(id)data electionAdvertisementDataByIds:(id)ids;

@end

#endif /* SCDASession_h */
