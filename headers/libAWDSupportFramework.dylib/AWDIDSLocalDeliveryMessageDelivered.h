//
//   Generated by https://github.com/blacktop/ipsw (Version: 3.1.685, BuildCommit: 61fde72ba2e055974329fddcad4020b94632d855)
//
//    - LC_BUILD_VERSION:  Platform: iOS, MinOS: 26.5, SDK: 26.5, Tool: ld (1267)
//    - LC_SOURCE_VERSION: 7019.0.0.0.0
//
#ifndef AWDIDSLocalDeliveryMessageDelivered_h
#define AWDIDSLocalDeliveryMessageDelivered_h
@import Foundation;

@class PBCodable;

@interface AWDIDSLocalDeliveryMessageDelivered : PBCodable <NSCopying> {
    /* instance variables */
    struct { unsigned int deliveryError:1; unsigned int isToDefaultPairedDevice:1; unsigned int linkType:1; unsigned int messageSize:1; unsigned int priority:1; unsigned int rTT:1; unsigned int timestamp:1; } _has;
}

@property (nonatomic) _Bool hasTimestamp;
@property (nonatomic) unsigned long long timestamp;
@property (readonly, nonatomic) _Bool hasService;
@property (retain, nonatomic) NSString *service;
@property (nonatomic) _Bool hasIsToDefaultPairedDevice;
@property (nonatomic) unsigned long long isToDefaultPairedDevice;
@property (nonatomic) _Bool hasMessageSize;
@property (nonatomic) unsigned long long messageSize;
@property (nonatomic) _Bool hasDeliveryError;
@property (nonatomic) unsigned long long deliveryError;
@property (nonatomic) _Bool hasRTT;
@property (nonatomic) unsigned long long rTT;
@property (nonatomic) _Bool hasLinkType;
@property (nonatomic) unsigned long long linkType;
@property (nonatomic) _Bool hasPriority;
@property (nonatomic) unsigned long long priority;

/* instance methods */
- (_Bool)readFrom:(id)from;
- (void)copyTo:(id)to;
- (unsigned long long)hash;
- (void)mergeFrom:(id)from;
- (id)copyWithZone:(struct _NSZone *)zone;
- (void)dealloc;
- (void)writeTo:(id)to;
- (id)dictionaryRepresentation;
- (id)description;
- (_Bool)isEqual:(id)equal;

@end

#endif /* AWDIDSLocalDeliveryMessageDelivered_h */
