//
//   Generated by https://github.com/blacktop/ipsw (Version: 3.1.685, BuildCommit: 61fde72ba2e055974329fddcad4020b94632d855)
//
//    - LC_BUILD_VERSION:  Platform: iOS, MinOS: 26.5, SDK: 26.5, Tool: ld (1267)
//    - LC_SOURCE_VERSION: 934.120.2.0.0
//
#ifndef OSASubmissionScheduler_h
#define OSASubmissionScheduler_h
@import Foundation;

@interface OSASubmissionScheduler : NSObject

/* class methods */
+ (void)_scheduleSubmissionWithPermissive:(_Bool)permissive;
+ (void)scheduleSubmission;
+ (void)_resetRetryCount;
+ (void)scheduleCleanupForUser:(id)user;
+ (void)_updateCadenceForActivity:(id)activity newCadenceParameters:(id)parameters;
+ (id)_getCurrentCadenceParametersFromActivity:(id)activity;
+ (long long)_retryCount;
+ (void)_incrementRetryCount;
+ (void)_saveLastSuccessTime;
+ (double)_lastSuccessTime;
+ (id)_calculateNewCadenceParametersWithPermissive:(_Bool)permissive fastLane:(_Bool)lane;

@end

#endif /* OSASubmissionScheduler_h */
