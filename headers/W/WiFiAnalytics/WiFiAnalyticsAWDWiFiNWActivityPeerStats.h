//
//   Generated by https://github.com/blacktop/ipsw (Version: 3.1.685, BuildCommit: 61fde72ba2e055974329fddcad4020b94632d855)
//
//    - LC_BUILD_VERSION:  Platform: iOS, MinOS: 26.5, SDK: 26.5, Tool: ld (1267)
//    - LC_SOURCE_VERSION: 805.6.0.0.0
//
#ifndef WiFiAnalyticsAWDWiFiNWActivityPeerStats_h
#define WiFiAnalyticsAWDWiFiNWActivityPeerStats_h
@import Foundation;

#include "WiFiAnalyticsAWDWiFiNWActivityTxCompletions.h"

@class PBCodable;

@interface WiFiAnalyticsAWDWiFiNWActivityPeerStats : PBCodable <NSCopying> {
    /* instance variables */
    struct { unsigned int ccaValue:1; unsigned int ibssCCA:1; unsigned int interferenceCCA:1; unsigned int nfValue:1; unsigned int obssCCA:1; unsigned int ofdmDesense:1; unsigned int qbssCCA:1; unsigned int qbssSTACount:1; unsigned int role:1; unsigned int rssiValue:1; unsigned int rssiValue0:1; unsigned int rssiValue1:1; unsigned int snrValue:1; } _has;
}

@property (nonatomic) _Bool hasRole;
@property (nonatomic) int role;
@property (readonly, nonatomic) _Bool hasHashID;
@property (retain, nonatomic) NSString *hashID;
@property (retain, nonatomic) NSMutableArray *rssis;
@property (retain, nonatomic) NSMutableArray *ccas;
@property (retain, nonatomic) NSMutableArray *snrs;
@property (retain, nonatomic) NSMutableArray *packets;
@property (retain, nonatomic) NSMutableArray *bytes;
@property (retain, nonatomic) NSMutableArray *txLatencyBKs;
@property (retain, nonatomic) NSMutableArray *txLatencyBEs;
@property (retain, nonatomic) NSMutableArray *txLatencyVOs;
@property (retain, nonatomic) NSMutableArray *txLatencyVIs;
@property (readonly, nonatomic) _Bool hasCompletions;
@property (retain, nonatomic) WiFiAnalyticsAWDWiFiNWActivityTxCompletions *completions;
@property (retain, nonatomic) NSMutableArray *acCompletions;
@property (nonatomic) _Bool hasRssiValue;
@property (nonatomic) int rssiValue;
@property (nonatomic) _Bool hasSnrValue;
@property (nonatomic) int snrValue;
@property (nonatomic) _Bool hasCcaValue;
@property (nonatomic) unsigned int ccaValue;
@property (nonatomic) _Bool hasNfValue;
@property (nonatomic) int nfValue;
@property (nonatomic) _Bool hasOfdmDesense;
@property (nonatomic) unsigned int ofdmDesense;
@property (nonatomic) _Bool hasIbssCCA;
@property (nonatomic) unsigned int ibssCCA;
@property (nonatomic) _Bool hasObssCCA;
@property (nonatomic) unsigned int obssCCA;
@property (nonatomic) _Bool hasInterferenceCCA;
@property (nonatomic) unsigned int interferenceCCA;
@property (nonatomic) _Bool hasQbssCCA;
@property (nonatomic) unsigned int qbssCCA;
@property (nonatomic) _Bool hasQbssSTACount;
@property (nonatomic) unsigned int qbssSTACount;
@property (nonatomic) _Bool hasRssiValue0;
@property (nonatomic) int rssiValue0;
@property (nonatomic) _Bool hasRssiValue1;
@property (nonatomic) int rssiValue1;

/* class methods */
+ (Class)snrType;
+ (Class)packetsType;
+ (Class)txLatencyBEType;
+ (Class)txLatencyVIType;
+ (Class)ccaType;
+ (Class)txLatencyBKType;
+ (Class)txLatencyVOType;
+ (Class)acCompletionsType;
+ (Class)bytesType;
+ (Class)rssiType;

/* instance methods */
- (_Bool)readFrom:(id)from;
- (void)copyTo:(id)to;
- (unsigned long long)hash;
- (void)mergeFrom:(id)from;
- (id)copyWithZone:(struct _NSZone *)zone;
- (void)writeTo:(id)to;
- (id)dictionaryRepresentation;
- (id)description;
- (_Bool)isEqual:(id)equal;
- (int)StringAsRole:(id)role;
- (id)roleAsString:(int)string;
- (id)packetsAtIndex:(unsigned long long)index;
- (void)addPackets:(id)packets;
- (void)clearPackets;
- (unsigned long long)packetsCount;
- (void)addSnr:(id)snr;
- (void)addTxLatencyBE:(id)be;
- (void)addTxLatencyVI:(id)vi;
- (void)addCca:(id)cca;
- (void)addTxLatencyBK:(id)bk;
- (void)addTxLatencyVO:(id)vo;
- (id)acCompletionsAtIndex:(unsigned long long)index;
- (unsigned long long)acCompletionsCount;
- (void)addAcCompletions:(id)completions;
- (void)addBytes:(id)bytes;
- (void)addRssi:(id)rssi;
- (id)bytesAtIndex:(unsigned long long)index;
- (unsigned long long)bytesCount;
- (id)ccaAtIndex:(unsigned long long)index;
- (unsigned long long)ccasCount;
- (void)clearAcCompletions;
- (void)clearBytes;
- (void)clearCcas;
- (void)clearRssis;
- (void)clearSnrs;
- (void)clearTxLatencyBEs;
- (void)clearTxLatencyBKs;
- (void)clearTxLatencyVIs;
- (void)clearTxLatencyVOs;
- (id)rssiAtIndex:(unsigned long long)index;
- (unsigned long long)rssisCount;
- (id)snrAtIndex:(unsigned long long)index;
- (unsigned long long)snrsCount;
- (id)txLatencyBEAtIndex:(unsigned long long)index;
- (unsigned long long)txLatencyBEsCount;
- (id)txLatencyBKAtIndex:(unsigned long long)index;
- (unsigned long long)txLatencyBKsCount;
- (id)txLatencyVIAtIndex:(unsigned long long)index;
- (unsigned long long)txLatencyVIsCount;
- (id)txLatencyVOAtIndex:(unsigned long long)index;
- (unsigned long long)txLatencyVOsCount;

@end

#endif /* WiFiAnalyticsAWDWiFiNWActivityPeerStats_h */
