//
//   Generated by https://github.com/blacktop/ipsw (Version: 3.1.685, BuildCommit: 61fde72ba2e055974329fddcad4020b94632d855)
//
//    - LC_BUILD_VERSION:  Platform: iOS, MinOS: 26.5, SDK: 26.5, Tool: ld (1267)
//    - LC_SOURCE_VERSION: 3525.4.1.0.0
//
#ifndef PLUSSchemaPLUSContactSuggesterSuggestionMetadataReported_h
#define PLUSSchemaPLUSContactSuggesterSuggestionMetadataReported_h
@import Foundation;

#include "SISchemaInstrumentationMessage.h"
#include "SISchemaUUID.h"

@interface PLUSSchemaPLUSContactSuggesterSuggestionMetadataReported : SISchemaInstrumentationMessage {
    /* instance variables */
    struct { unsigned int isDuplicateSuggestion:1; unsigned int isSuggestionPresentInModelOutput:1; } _has;
}

@property (retain, nonatomic) SISchemaUUID *suggestionId;
@property (nonatomic) _Bool hasSuggestionId;
@property (nonatomic) _Bool isDuplicateSuggestion;
@property (nonatomic) _Bool hasIsDuplicateSuggestion;
@property (nonatomic) _Bool isSuggestionPresentInModelOutput;
@property (nonatomic) _Bool hasIsSuggestionPresentInModelOutput;
@property (readonly, nonatomic) NSData *jsonData;

/* instance methods */
- (_Bool)readFrom:(id)from;
- (unsigned long long)hash;
- (id)applySensitiveConditionsPolicy:(id)policy;
- (void)writeTo:(id)to;
- (id)initWithJSON:(id)json;
- (id)suppressMessageUnderConditions;
- (id)initWithDictionary:(id)dictionary;
- (id)dictionaryRepresentation;
- (_Bool)isEqual:(id)equal;
- (void)deleteIsDuplicateSuggestion;
- (void)deleteIsSuggestionPresentInModelOutput;
- (void)deleteSuggestionId;

@end

#endif /* PLUSSchemaPLUSContactSuggesterSuggestionMetadataReported_h */
