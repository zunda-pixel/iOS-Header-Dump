//
//   Generated by https://github.com/blacktop/ipsw (Version: 3.1.685, BuildCommit: 61fde72ba2e055974329fddcad4020b94632d855)
//
//    - LC_BUILD_VERSION:  Platform: iOS, MinOS: 26.5, SDK: 26.5, Tool: ld (1267)
//    - LC_SOURCE_VERSION: 3525.4.1.0.0
//
#ifndef IFTSchemaIFTSystemResponse_h
#define IFTSchemaIFTSystemResponse_h
@import Foundation;

#include "IFTSchemaIFTStatementId.h"
#include "IFTSchemaIFTStatementOutcome.h"
#include "SISchemaInstrumentationMessage.h"

@interface IFTSchemaIFTSystemResponse : SISchemaInstrumentationMessage {
    /* instance variables */
    struct { unsigned int exists:1; } _has;
}

@property (nonatomic) _Bool exists;
@property (nonatomic) _Bool hasExists;
@property (retain, nonatomic) IFTSchemaIFTStatementOutcome *outcome;
@property (nonatomic) _Bool hasOutcome;
@property (retain, nonatomic) IFTSchemaIFTStatementId *statementId;
@property (nonatomic) _Bool hasStatementId;
@property (copy, nonatomic) NSString *toolId;
@property (nonatomic) _Bool hasToolId;
@property (copy, nonatomic) NSArray *interpretedStatementResults;
@property (readonly, nonatomic) NSData *jsonData;

/* instance methods */
- (_Bool)readFrom:(id)from;
- (unsigned long long)hash;
- (id)applySensitiveConditionsPolicy:(id)policy;
- (void)writeTo:(id)to;
- (id)initWithJSON:(id)json;
- (id)suppressMessageUnderConditions;
- (id)initWithDictionary:(id)dictionary;
- (id)dictionaryRepresentation;
- (void)deleteExists;
- (_Bool)isEqual:(id)equal;
- (void)addInterpretedStatementResults:(id)results;
- (void)clearInterpretedStatementResults;
- (void)deleteInterpretedStatementResults;
- (void)deleteOutcome;
- (void)deleteStatementId;
- (void)deleteToolId;
- (id)interpretedStatementResultsAtIndex:(unsigned long long)index;
- (unsigned long long)interpretedStatementResultsCount;

@end

#endif /* IFTSchemaIFTSystemResponse_h */
