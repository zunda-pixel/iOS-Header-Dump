//
//   Generated by https://github.com/blacktop/ipsw (Version: 3.1.685, BuildCommit: 61fde72ba2e055974329fddcad4020b94632d855)
//
//    - LC_BUILD_VERSION:  Platform: iOS, MinOS: 26.5, SDK: 26.5, Tool: ld (1267)
//    - LC_SOURCE_VERSION: 2360.120.2.0.0
//
#ifndef CKDPAdopterCapabilitiesCheckResponse_h
#define CKDPAdopterCapabilitiesCheckResponse_h
@import Foundation;

#include "CKDPSignedVersionedBlob.h"

@class PBCodable;

@interface CKDPAdopterCapabilitiesCheckResponse : PBCodable <NSCopying> {
    /* instance variables */
    struct { unsigned int isSupported:1; } _has;
}

@property (nonatomic) _Bool hasIsSupported;
@property (nonatomic) _Bool isSupported;
@property (readonly, nonatomic) _Bool hasSignedSupportedAdopterCapabilities;
@property (retain, nonatomic) CKDPSignedVersionedBlob *signedSupportedAdopterCapabilities;
@property (readonly, nonatomic) _Bool hasSignedShareUsage;
@property (retain, nonatomic) CKDPSignedVersionedBlob *signedShareUsage;
@property (readonly, nonatomic) _Bool hasSignedZoneUsage;
@property (retain, nonatomic) CKDPSignedVersionedBlob *signedZoneUsage;
@property (readonly, nonatomic) _Bool hasContinuation;
@property (retain, nonatomic) NSData *continuation;

/* instance methods */
- (_Bool)readFrom:(id)from;
- (void)copyTo:(id)to;
- (unsigned long long)hash;
- (void)mergeFrom:(id)from;
- (id)copyWithZone:(struct _NSZone *)zone;
- (void)writeTo:(id)to;
- (id)dictionaryRepresentation;
- (id)description;
- (_Bool)isEqual:(id)equal;

@end

#endif /* CKDPAdopterCapabilitiesCheckResponse_h */
