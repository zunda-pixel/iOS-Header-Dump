//
//   Generated by https://github.com/blacktop/ipsw (Version: 3.1.685, BuildCommit: 61fde72ba2e055974329fddcad4020b94632d855)
//
//    - LC_BUILD_VERSION:  Platform: iOS, MinOS: 26.5, SDK: 26.5, Tool: ld (1267)
//    - LC_SOURCE_VERSION: 4025.610.1.0.0
//
#ifndef _MRDiscoverySessionConfigurationProtobuf_h
#define _MRDiscoverySessionConfigurationProtobuf_h
@import Foundation;

@class PBCodable;

@interface _MRDiscoverySessionConfigurationProtobuf : PBCodable <NSCopying> {
    /* instance variables */
    struct { unsigned int features:1; unsigned int targetSessionID:1; unsigned int alwaysAllowUpdates:1; unsigned int cachedDiscoveryEnabled:1; unsigned int enableThrottling:1; unsigned int populatesExternalDevice:1; } _has;
}

@property (nonatomic) _Bool hasFeatures;
@property (nonatomic) int features;
@property (readonly, nonatomic) _Bool hasRoutingContextUID;
@property (retain, nonatomic) NSString *routingContextUID;
@property (nonatomic) _Bool hasEnableThrottling;
@property (nonatomic) _Bool enableThrottling;
@property (nonatomic) _Bool hasAlwaysAllowUpdates;
@property (nonatomic) _Bool alwaysAllowUpdates;
@property (nonatomic) _Bool hasPopulatesExternalDevice;
@property (nonatomic) _Bool populatesExternalDevice;
@property (readonly, nonatomic) _Bool hasOutputDeviceUID;
@property (retain, nonatomic) NSString *outputDeviceUID;
@property (nonatomic) _Bool hasTargetSessionID;
@property (nonatomic) unsigned int targetSessionID;
@property (nonatomic) _Bool hasCachedDiscoveryEnabled;
@property (nonatomic) _Bool cachedDiscoveryEnabled;

/* instance methods */
- (_Bool)readFrom:(id)from;
- (void)copyTo:(id)to;
- (unsigned long long)hash;
- (void)mergeFrom:(id)from;
- (id)copyWithZone:(struct _NSZone *)zone;
- (void)writeTo:(id)to;
- (id)dictionaryRepresentation;
- (id)description;
- (_Bool)isEqual:(id)equal;
- (int)StringAsFeatures:(id)features;
- (id)featuresAsString:(int)string;

@end

#endif /* _MRDiscoverySessionConfigurationProtobuf_h */
