//
//   Generated by https://github.com/blacktop/ipsw (Version: 3.1.685, BuildCommit: 61fde72ba2e055974329fddcad4020b94632d855)
//
//    - LC_BUILD_VERSION:  Platform: iOS, MinOS: 26.5, SDK: 26.5, Tool: ld (1267)
//    - LC_SOURCE_VERSION: 165.0.0.0.0
//
#ifndef PSPointerPortalSourceCollection_h
#define PSPointerPortalSourceCollection_h
@import Foundation;

#include "PSMatchMoveSource.h"
#include "PSPortalSource.h"

@interface PSPointerPortalSourceCollection : NSObject <NSCopying, NSSecureCoding>

@property (readonly, nonatomic) unsigned int sourceContextID;
@property (nonatomic) _Bool shouldBeInvalidatedIfPointerLeavesDisplay;
@property (readonly, copy, nonatomic) PSPortalSource *overlayEffectPortalSource;
@property (readonly, copy, nonatomic) PSPortalSource *pointerPortalSource;
@property (readonly, copy, nonatomic) PSMatchMoveSource *samplingMatchMoveSource;

/* class methods */
+ (_Bool)supportsSecureCoding;

/* instance methods */
- (unsigned long long)hash;
- (id)copyWithZone:(struct _NSZone *)zone;
- (id)initWithCoder:(id)coder;
- (void)encodeWithCoder:(id)coder;
- (id)description;
- (_Bool)isEqual:(id)equal;
- (id)initWithPointerPortalSource:(id)source overlayEffectPortalSource:(id)source samplingMatchMoveSource:(id)source;

@end

#endif /* PSPointerPortalSourceCollection_h */
