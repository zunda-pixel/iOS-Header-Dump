//
//   Generated by https://github.com/blacktop/ipsw (Version: 3.1.685, BuildCommit: 61fde72ba2e055974329fddcad4020b94632d855)
//
//    - LC_BUILD_VERSION:  Platform: iOS, MinOS: 26.5, SDK: 26.5, Tool: ld (1267)
//    - LC_SOURCE_VERSION: 7019.0.0.0.0
//
#ifndef AWDWiFiSlowWiFiReport_h
#define AWDWiFiSlowWiFiReport_h
@import Foundation;

#include "AWDSlowWiFiNotification.h"

@class PBCodable;

@interface AWDWiFiSlowWiFiReport : PBCodable <NSCopying> {
    /* instance variables */
    struct { unsigned int timestamp:1; } _has;
}

@property (nonatomic) _Bool hasTimestamp;
@property (nonatomic) unsigned long long timestamp;
@property (readonly, nonatomic) _Bool hasSlowNotice;
@property (retain, nonatomic) AWDSlowWiFiNotification *slowNotice;
@property (retain, nonatomic) NSMutableArray *linkQualSamples;
@property (retain, nonatomic) NSMutableArray *usbEvents;

/* class methods */
+ (Class)linkQualSampleType;
+ (Class)usbEventType;

/* instance methods */
- (_Bool)readFrom:(id)from;
- (void)copyTo:(id)to;
- (unsigned long long)hash;
- (void)mergeFrom:(id)from;
- (id)copyWithZone:(struct _NSZone *)zone;
- (void)dealloc;
- (void)writeTo:(id)to;
- (id)dictionaryRepresentation;
- (id)description;
- (_Bool)isEqual:(id)equal;
- (void)addLinkQualSample:(id)sample;
- (void)addUsbEvent:(id)event;
- (void)clearLinkQualSamples;
- (void)clearUsbEvents;
- (id)linkQualSampleAtIndex:(unsigned long long)index;
- (unsigned long long)linkQualSamplesCount;
- (id)usbEventAtIndex:(unsigned long long)index;
- (unsigned long long)usbEventsCount;

@end

#endif /* AWDWiFiSlowWiFiReport_h */
