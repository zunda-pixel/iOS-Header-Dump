//
//   Generated by https://github.com/blacktop/ipsw (Version: 3.1.685, BuildCommit: 61fde72ba2e055974329fddcad4020b94632d855)
//
//    - LC_BUILD_VERSION:  Platform: iOS, MinOS: 26.5, SDK: 26.5, Tool: ld (1267)
//    - LC_SOURCE_VERSION: 1642.6.5.0.0
//
#ifndef PKPeerPaymentMessageComposeController_h
#define PKPeerPaymentMessageComposeController_h
@import Foundation;

#include "MFMessageComposeViewControllerDelegate-Protocol.h"

@class MSMessage, UIViewController;

@interface PKPeerPaymentMessageComposeController : NSObject <MFMessageComposeViewControllerDelegate>

@property (weak, nonatomic) UIViewController *currentHostController;
@property (readonly, nonatomic) MSMessage *message;
@property (readonly, copy, nonatomic) NSString *recipientAddress;
@property (copy, nonatomic) id /* block */ completionHandler;
@property (readonly) unsigned long long hash;
@property (readonly) Class superclass;
@property (readonly, copy) NSString *description;
@property (readonly, copy) NSString *debugDescription;

/* instance methods */
- (void)messageComposeViewController:(id)controller didFinishWithResult:(long long)result;
- (id)initWithMessage:(id)message recipientAddress:(id)address;
- (void)presentMessageOverController:(id)controller presentationHandler:(id /* block */)handler;

@end

#endif /* PKPeerPaymentMessageComposeController_h */
