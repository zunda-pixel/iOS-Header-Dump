//
//   Generated by https://github.com/blacktop/ipsw (Version: 3.1.685, BuildCommit: 61fde72ba2e055974329fddcad4020b94632d855)
//
//    - LC_BUILD_VERSION:  Platform: iOS, MinOS: 26.5, SDK: 26.5, Tool: ld (1267)
//    - LC_SOURCE_VERSION: 424.15.0.0.0
//
#ifndef SCROBrailleUIDisplayManager_h
#define SCROBrailleUIDisplayManager_h
@import Foundation;

#include "SCROBrailleUIDisplayManagerSpeechDelegate-Protocol.h"
#include "SCROBrailleUIView.h"

@interface SCROBrailleUIDisplayManager : NSObject

@property (retain, nonatomic) NSMutableArray *lineQueue;
@property (retain, nonatomic) NSMutableArray *viewQueue;
@property (retain, nonatomic) id <SCROBrailleUIDisplayManagerSpeechDelegate> speechDelegate;
@property (readonly, nonatomic) SCROBrailleUIView *visibleView;
@property (readonly, nonatomic) _Bool isHidingViews;

/* class methods */
+ (id)sharedManager;

/* instance methods */
- (id)init;
- (id)_brailleUILineForCurrentBraille;
- (void)_commitDisplayedBrailleToTopMostView;
- (_Bool)_isFirstViewEqualTo:(id)to;
- (void)_speakFocusedListItem;
- (void)_updateBrailleModel;
- (void)_updateIsHidingViews;
- (void)dismissAll;
- (void)dismissLineForView:(id)view;
- (void)display:(id)display forView:(id)view clearAtNextDotPress:(_Bool)press;
- (void)hideViews;
- (id)lineForView:(id)view;
- (void)requestSpeech:(id)speech language:(id)language shouldQueue:(_Bool)queue;
- (void)showFirstLineForView:(id)view;
- (void)showLastLineForView:(id)view;
- (void)showNextLineForView:(id)view;
- (void)showPreviousLineForView:(id)view;
- (void)showViews;

@end

#endif /* SCROBrailleUIDisplayManager_h */
