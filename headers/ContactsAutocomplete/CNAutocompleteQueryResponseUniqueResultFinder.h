//
//   Generated by https://github.com/blacktop/ipsw (Version: 3.1.685, BuildCommit: 61fde72ba2e055974329fddcad4020b94632d855)
//
//    - LC_BUILD_VERSION:  Platform: iOS, MinOS: 26.5, SDK: 26.5, Tool: ld (1267)
//    - LC_SOURCE_VERSION: 1356.500.41.0.0
//
#ifndef CNAutocompleteQueryResponseUniqueResultFinder_h
#define CNAutocompleteQueryResponseUniqueResultFinder_h
@import Foundation;

@interface CNAutocompleteQueryResponseUniqueResultFinder : NSObject {
    /* instance variables */
    NSMutableArray *_uniqueResults;
    NSMutableArray *_hashes;
    NSDictionary *_hashIndex;
    NSIndexSet *_firstIndexes;
    NSMutableIndexSet *_uniqueResultIndexes;
    id /* block */ _duplicateResultHandler;
}

/* class methods */
+ (id)findUniqueResults:(id)results duplicateResultHandler:(id /* block */)handler;

/* instance methods */
- (void)removeDuplicateResults;
- (void)resolveDuplicatesWithinIndexes:(id)indexes;
- (id)findUniqueResults;
- (void)generateHashes;
- (void)indexHashes;
- (id)initWithResults:(id)results duplicateResultHandler:(id /* block */)handler;

@end

#endif /* CNAutocompleteQueryResponseUniqueResultFinder_h */
