//
//   Generated by https://github.com/blacktop/ipsw (Version: 3.1.685, BuildCommit: 61fde72ba2e055974329fddcad4020b94632d855)
//
//    - LC_BUILD_VERSION:  Platform: iOS, MinOS: 26.5, SDK: 26.5, Tool: ld (1267)
//    - LC_SOURCE_VERSION: 4557.5.7.101.0
//
#ifndef SBSystemStatusBatteryDataProvider_h
#define SBSystemStatusBatteryDataProvider_h
@import Foundation;

#include "BCBatteryDeviceObserving-Protocol.h"
#include "BSInvalidatable-Protocol.h"

@class BCBatteryDeviceController, STBatteryStatusDomainData, STBatteryStatusDomainPublisher;

@interface SBSystemStatusBatteryDataProvider : NSObject <BCBatteryDeviceObserving, BSInvalidatable>

@property (readonly, nonatomic) STBatteryStatusDomainPublisher *batteryDataPublisher;
@property (readonly, nonatomic) BCBatteryDeviceController *batteryDeviceController;
@property (nonatomic) _Bool shouldExposeNotChargingState;
@property (readonly, nonatomic) STBatteryStatusDomainData *lastPublishedData;
@property (readonly) unsigned long long hash;
@property (readonly) Class superclass;
@property (readonly, copy) NSString *description;
@property (readonly, copy) NSString *debugDescription;

/* instance methods */
- (void)_registerForNotifications;
- (void)connectedDevicesDidChange:(id)change;
- (void)_updateShouldExposeNotChargingState;
- (_Bool)_isInNotChargingState;
- (void)invalidate;
- (void)dealloc;
- (void)_noteNotChargingStatusChanged;
- (void)_updateData;
- (id)init;

@end

#endif /* SBSystemStatusBatteryDataProvider_h */
