//
//   Generated by https://github.com/blacktop/ipsw (Version: 3.1.685, BuildCommit: 61fde72ba2e055974329fddcad4020b94632d855)
//
//    - LC_BUILD_VERSION:  Platform: iOS, MinOS: 26.5, SDK: 26.5, Tool: ld (1267)
//    - LC_SOURCE_VERSION: 405.3.1.0.0
//
#ifndef VCPFaceTensorModel_h
#define VCPFaceTensorModel_h
@import Foundation;

@interface VCPFaceTensorModel : NSObject {
    /* instance variables */
    int _numBlendshapePlusOne;
    int _numComponents;
    int _numIdentities;
    float * _transformedCoeff;
    float * _blendShapeDelta;
}

@property (readonly) int numVertices;
@property (readonly) float * meanBlendshape;
@property (readonly) float * tensorCoeff;
@property (readonly) float * componentsBlendshape;
@property (readonly) int * blendshapeComponentIndex;

/* instance methods */
- (void)dealloc;
- (void)calculateMesh:(float *)mesh numVertices:(int)vertices blendshapes:(float *)blendshapes outputMesh:(float *)mesh;
- (void)calculateModelBlendshapes:(float *)blendshapes outputBlendshapes:(float *)blendshapes;
- (id)initWithModelFile:(struct __sFILE { char *x0; int x1; int x2; short x3; short x4; struct __sbuf { char *x0; int x1; } x5; int x6; void *x7; void * /* function */ x8; void * /* function */ x9; void * /* function */ x10; void * /* function */ x11; struct __sbuf { char *x0; int x1; } x12; struct __sFILEX *x13; int x14; unsigned char x15[3]; unsigned char x16[1]; struct __sbuf { char *x0; int x1; } x17; int x18; long long x19; } *)file;

@end

#endif /* VCPFaceTensorModel_h */
