//
//   Generated by https://github.com/blacktop/ipsw (Version: 3.1.685, BuildCommit: 61fde72ba2e055974329fddcad4020b94632d855)
//
//    - LC_BUILD_VERSION:  Platform: iOS, MinOS: 26.5, SDK: 26.5, Tool: ld (1267)
//    - LC_SOURCE_VERSION: 6200.6.8.2.1
//
#ifndef HKHRSampleClassification_h
#define HKHRSampleClassification_h
@import Foundation;

@interface HKHRSampleClassification : NSObject <NSCopying, NSSecureCoding>

@property (readonly, nonatomic) NSUUID *sampleUUID;
@property (readonly, nonatomic) long long classification;
@property (readonly, nonatomic) NSDateInterval *sampleDateInterval;
@property (readonly, nonatomic) NSTimeZone *sampleTimeZone;

/* class methods */
+ (_Bool)supportsSecureCoding;

/* instance methods */
- (unsigned long long)hash;
- (id)copyWithZone:(struct _NSZone *)zone;
- (id)initWithCoder:(id)coder;
- (void)encodeWithCoder:(id)coder;
- (id)description;
- (_Bool)isEqual:(id)equal;
- (id)initWithSampleUUID:(id)uuid classification:(long long)classification sampleDateInterval:(id)interval sampleTimeZone:(id)zone;

@end

#endif /* HKHRSampleClassification_h */
