//
//   Generated by https://github.com/blacktop/ipsw (Version: 3.1.685, BuildCommit: 61fde72ba2e055974329fddcad4020b94632d855)
//
//    - LC_BUILD_VERSION:  Platform: iOS, MinOS: 26.5, SDK: 26.5, Tool: ld (1267)
//    - LC_SOURCE_VERSION: 2031.35.2.9.20
//
#ifndef GEORevealedPlaceCardModuleButton_h
#define GEORevealedPlaceCardModuleButton_h
@import Foundation;

@class PBCodable, PBDataReader, PBUnknownFields;

@interface GEORevealedPlaceCardModuleButton : PBCodable <NSCopying> {
    /* instance variables */
    PBDataReader *_reader;
    unsigned int _readerMarkPos;
    unsigned int _readerMarkLength;
    struct os_unfair_lock_s { unsigned int _os_unfair_lock_opaque; } _readerLock;
    struct { unsigned int has_linkType:1; unsigned int has_type:1; unsigned int has_isGroup:1; unsigned int read_unknownFields:1; unsigned int read_classification:1; unsigned int read_metadata:1; unsigned int read_vendor:1; unsigned int wrote_anyField:1; } _flags;
}

@property (nonatomic) _Bool hasType;
@property (nonatomic) int type;
@property (nonatomic) _Bool hasIsGroup;
@property (nonatomic) _Bool isGroup;
@property (readonly, nonatomic) _Bool hasMetadata;
@property (retain, nonatomic) NSString *metadata;
@property (readonly, nonatomic) _Bool hasVendor;
@property (retain, nonatomic) NSString *vendor;
@property (nonatomic) _Bool hasLinkType;
@property (nonatomic) int linkType;
@property (readonly, nonatomic) _Bool hasClassification;
@property (retain, nonatomic) NSString *classification;
@property (readonly, nonatomic) PBUnknownFields *unknownFields;

/* class methods */
+ (_Bool)isValid:(id)valid;
+ (id)moduleButtonForType:(int)type;

/* instance methods */
- (_Bool)readFrom:(id)from;
- (id)initWithData:(id)data;
- (void)copyTo:(id)to;
- (unsigned long long)hash;
- (void)mergeFrom:(id)from;
- (id)copyWithZone:(struct _NSZone *)zone;
- (id)jsonRepresentation;
- (void)writeTo:(id)to;
- (id)typeAsString:(int)string;
- (id)initWithJSON:(id)json;
- (id)init;
- (id)initWithDictionary:(id)dictionary;
- (void)readAll:(_Bool)all;
- (id)dictionaryRepresentation;
- (id)description;
- (void)clearUnknownFields:(_Bool)fields;
- (int)StringAsType:(id)type;
- (_Bool)isEqual:(id)equal;
- (int)StringAsLinkType:(id)type;
- (id)linkTypeAsString:(int)string;

@end

#endif /* GEORevealedPlaceCardModuleButton_h */
