//
//   Generated by https://github.com/blacktop/ipsw (Version: 3.1.685, BuildCommit: 61fde72ba2e055974329fddcad4020b94632d855)
//
//    - LC_BUILD_VERSION:  Platform: iOS, MinOS: 26.5, SDK: 26.5, Tool: ld (1267)
//    - LC_SOURCE_VERSION: 2031.35.2.9.20
//
#ifndef GEOWiFiHarvestLabel_h
#define GEOWiFiHarvestLabel_h
@import Foundation;

@class PBCodable, PBDataReader;

@interface GEOWiFiHarvestLabel : PBCodable <NSCopying> {
    /* instance variables */
    PBDataReader *_reader;
    unsigned int _readerMarkPos;
    unsigned int _readerMarkLength;
    struct os_unfair_lock_s { unsigned int _os_unfair_lock_opaque; } _readerLock;
    struct { unsigned int has_networkType:1; unsigned int has_originator:1; unsigned int read_comment:1; unsigned int read_internalIdentifier:1; unsigned int read_venueCategory:1; unsigned int wrote_anyField:1; } _flags;
}

@property (nonatomic) _Bool hasOriginator;
@property (nonatomic) int originator;
@property (readonly, nonatomic) _Bool hasVenueCategory;
@property (retain, nonatomic) NSString *venueCategory;
@property (readonly, nonatomic) _Bool hasComment;
@property (retain, nonatomic) NSString *comment;
@property (nonatomic) _Bool hasNetworkType;
@property (nonatomic) int networkType;
@property (readonly, nonatomic) _Bool hasInternalIdentifier;
@property (retain, nonatomic) NSString *internalIdentifier;

/* class methods */
+ (_Bool)isValid:(id)valid;

/* instance methods */
- (_Bool)readFrom:(id)from;
- (id)initWithData:(id)data;
- (void)copyTo:(id)to;
- (unsigned long long)hash;
- (void)mergeFrom:(id)from;
- (id)copyWithZone:(struct _NSZone *)zone;
- (id)jsonRepresentation;
- (void)writeTo:(id)to;
- (id)initWithJSON:(id)json;
- (id)init;
- (id)initWithDictionary:(id)dictionary;
- (void)readAll:(_Bool)all;
- (id)dictionaryRepresentation;
- (id)description;
- (_Bool)isEqual:(id)equal;
- (int)StringAsOriginator:(id)originator;
- (int)StringAsNetworkType:(id)type;
- (id)networkTypeAsString:(int)string;
- (id)originatorAsString:(int)string;

@end

#endif /* GEOWiFiHarvestLabel_h */
