//
//   Generated by https://github.com/blacktop/ipsw (Version: 3.1.685, BuildCommit: 61fde72ba2e055974329fddcad4020b94632d855)
//
//    - LC_BUILD_VERSION:  Platform: iOS, MinOS: 26.5, SDK: 26.5, Tool: ld (1267)
//    - LC_SOURCE_VERSION: 1145.40.5.0.0
//
#ifndef TVModalPresenterConfiguration_h
#define TVModalPresenterConfiguration_h
@import Foundation;

@class UIView;

@interface TVModalPresenterConfiguration : NSObject

@property (nonatomic) _Bool allowsModalOverModal;
@property (nonatomic) _Bool allowsMenuDismissal;
@property (copy, nonatomic) NSString *barButtonID;
@property (nonatomic) unsigned long long configurationType;
@property (nonatomic) struct CGSize { double x0; double x1; } formSheetSize;
@property (nonatomic) struct CGSize { double x0; double x1; } popoverSize;
@property (nonatomic) _Bool navigationBarHidden;
@property (nonatomic) long long presentationStyle;
@property (retain, nonatomic) UIView *sourceView;
@property (nonatomic) struct CGRect { struct CGPoint { double x0; double x1; } x0; struct CGSize { double x0; double x1; } x1; } sourceRect;

/* instance methods */
- (id)init;
- (id)initWithDictionary:(id)dictionary;
- (void)_configureWithDictionary:(id)dictionary;

@end

#endif /* TVModalPresenterConfiguration_h */
