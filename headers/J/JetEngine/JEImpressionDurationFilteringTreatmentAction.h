//
//   Generated by https://github.com/blacktop/ipsw (Version: 3.1.685, BuildCommit: 61fde72ba2e055974329fddcad4020b94632d855)
//
//    - LC_BUILD_VERSION:  Platform: iOS, MinOS: 26.5, SDK: 26.5, Tool: ld (1267)
//    - LC_SOURCE_VERSION: 9.5.5.0.0
//
#ifndef JEImpressionDurationFilteringTreatmentAction_h
#define JEImpressionDurationFilteringTreatmentAction_h
@import Foundation;

#include "JETreatmentAction.h"

@interface JEImpressionDurationFilteringTreatmentAction : JETreatmentAction

@property (readonly, nonatomic) long long durationThreshold;
@property (readonly, nonatomic) NSPredicate *filters;

/* instance methods */
- (id)filteredDurationArray:(id)array;
- (id)filteredViewedInfoDetailed:(id)detailed;
- (id)initWithField:(id)field configuration:(id)configuration;
- (id)performAction:(id)action context:(id)context;
- (id)updatedImpression:(id)impression;

@end

#endif /* JEImpressionDurationFilteringTreatmentAction_h */
