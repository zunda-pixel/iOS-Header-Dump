//
//   Generated by https://github.com/blacktop/ipsw (Version: 3.1.685, BuildCommit: 61fde72ba2e055974329fddcad4020b94632d855)
//
//    - LC_BUILD_VERSION:  Platform: iOS, MinOS: 26.5, SDK: 26.5, Tool: ld (1267)
//    - LC_SOURCE_VERSION: 251.35.2.9.11
//
#ifndef MUScrollableStackView_h
#define MUScrollableStackView_h
@import Foundation;

#include "MUContentStackViewProtocol-Protocol.h"
#include "MUStackView.h"

@class NSLayoutConstraint, UIScrollView;

@interface MUScrollableStackView : UIScrollView <MUContentStackViewProtocol> {
    /* instance variables */
    MUStackView *_stackView;
    NSLayoutConstraint *_axisConstraint;
    struct CGSize { double width; double height; } _cachedContentSize;
}

@property (readonly, nonatomic) _Bool primaryAxisCentering;
@property (nonatomic) long long alignment;
@property (nonatomic) long long axis;
@property (nonatomic) long long distribution;
@property (nonatomic) double spacing;
@property (nonatomic) struct UIEdgeInsets { double x0; double x1; double x2; double x3; } contentEdgeInsets;
@property (readonly, copy, nonatomic) NSArray *arrangedSubviews;
@property (readonly) unsigned long long hash;
@property (readonly) Class superclass;
@property (readonly, copy) NSString *description;
@property (readonly, copy) NSString *debugDescription;

/* instance methods */
- (struct CGSize { double x0; double x1; })systemLayoutSizeFittingSize:(struct CGSize { double x0; double x1; })size withHorizontalFittingPriority:(float)priority verticalFittingPriority:(float)priority;
- (void)_setupViews;
- (void)setContentSize:(struct CGSize { double x0; double x1; })size;
- (struct CGSize { double x0; double x1; })systemLayoutSizeFittingSize:(struct CGSize { double x0; double x1; })size;
- (void)_commonInit;
- (struct CGSize { double x0; double x1; })sizeThatFits:(struct CGSize { double x0; double x1; })fits;
- (id)initWithFrame:(struct CGRect { struct CGPoint { double x0; double x1; } x0; struct CGSize { double x0; double x1; } x1; })frame;
- (void)layoutSubviews;
- (struct CGSize { double x0; double x1; })intrinsicContentSize;
- (_Bool)insetsLayoutMarginsFromSafeArea;
- (void)addArrangedSubview:(id)subview;
- (void)insertArrangedSubview:(id)subview atIndex:(unsigned long long)index;
- (void)removeArrangedSubview:(id)subview;
- (void)setCustomSpacing:(double)spacing afterView:(id)view;
- (void)setInsetsLayoutMarginsFromSafeArea:(_Bool)area;
- (void)_updateForHorizontalAxisIfNeeded;
- (void)_updateOrientationWithNewAxis:(long long)axis;
- (id)addArrangedCardShadowViewForView:(id)view;
- (void)addArrangedSubview:(id)subview withCustomSpacing:(double)spacing;
- (id)initWithPrimaryAxisCentering:(_Bool)centering;

@end

#endif /* MUScrollableStackView_h */
