//
//   Generated by https://github.com/blacktop/ipsw (Version: 3.1.685, BuildCommit: 61fde72ba2e055974329fddcad4020b94632d855)
//
//    - LC_BUILD_VERSION:  Platform: iOS, MinOS: 26.5, SDK: 26.5, Tool: ld (1267)
//    - LC_SOURCE_VERSION: 59.120.4.0.0
//
#ifndef MCUIAppSignerUninstallerUtilities_h
#define MCUIAppSignerUninstallerUtilities_h
@import Foundation;

@interface MCUIAppSignerUninstallerUtilities : NSObject

/* class methods */
+ (void)_asyncUninstallApplicationsForAppsigner:(id)appsigner withUninstaller:(id)uninstaller dispatchGroup:(id)group;
+ (void)_asyncUninstallProvisioningProfilesForAppSigner:(id)signer withUninstaller:(id)uninstaller dispatchGroup:(id)group;
+ (id)_provisioningProfileUUIDsForAppSigner:(id)signer;
+ (void)uninstallAppSigner:(id)signer withUninstaller:(id)uninstaller dispatchGroup:(id)group completion:(id /* block */)completion;

@end

#endif /* MCUIAppSignerUninstallerUtilities_h */
